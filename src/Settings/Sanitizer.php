...
class Sanitizer {
	...
	public function sanitize( $raw ): array {
		...
		return array(
			...
			'ai_image_alt_mode'          => ( static function () use ( $raw ) {
				$mode = sanitize_key( (string) ( $raw['ai_image_alt_mode'] ?? 'keyword' ) );
				return in_array( $mode, array( 'keyword', 'descriptive' ), true ) ? $mode : 'keyword';
			} )(),
